...
function findGrape(arr) {
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].color === "purple") {
            return `The fruit with index ${arr.indexOf(arr[i])} is a grape`
        }
    }
}
const fruits = [
...
// CODE HERE
const dogs = (param1, param2) => `The first parameter is ${param1}. The second parameter is ${param2}`

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
// console.log(dogs('Penny', 'Scotch'))
...
greeting('Maria', 'Reeves', fullName)
...
function pricesPlusTax(prices, cb) {
    for (let i = 0; i < prices.length; i++) {
        let newTotal = prices[i] + (prices[i] * 0.2)
