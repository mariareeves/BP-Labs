...
// contains(names, 'Maria', result => {
//   if (result === true) {
//     console.log('Maria is in the array')
//   } else {
//     console.log('Maria is not in the array')
//   }
// })
...
function uniq(arr, cb) {
  for (let i = 0; i < arr.length; i++) {
    // console.log(`this is i: ${i} and the value ${arr[i]}`)
    for (let j = i + 1; j < arr.length; j++) {
      // console.log(`this is j: ${j} and the value ${arr[j]}`)
      // console.log(`this is arr[i]: ${arr[i]} and arr[j]${arr[j]}`)
      if (arr[i] === arr[j]) {
        arr.splice(j, 1)
        j--
      }
...
// CODE HERE
each(names, cb = (item, i) => console.log(`The item at index ${i} is ${item}`))
