...
const repeatMyParam = () => {
    ...
    axios.get(`${baseURL}/repeat/${str}`)
        .then(res => {
            ...
            const repeatPTag = document.querySelector('#repeat-text')
            repeatPTag.textContent = res.data
            repeatPTag.style.display = 'block'
        })
        .catch(err => {
            console.log(err)
        })
...
// CODE HERE
function sendQuery() {
    const query = 'this is confusing'
    const query2 = ' and crazy'
    axios.get(`${baseURL}/query-test?${query}&${query2}`)
        .then(res => {
            console.log(res.data)
        })
        .catch(err => {
...
/*
    ...
    he server receveid the GET request without any query parameters. On my terminal, I got a message error saying 'Error [ERR_HTTP_HEADERS_SENT]: Cannot set headers after they are sent to the client'. On my browser console, I got a msg saying 'You sent an empty query' 


    2: Send more than 1 query on the URL -- what happened?
    On my console in the browser I get the following consolelog: {message: 'You sent more than 1 query!', queries: {…}}
    message
    : 
